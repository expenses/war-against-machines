...
impl Interface {
    pub fn new() -> Self {
        Self {
            ...
            unit_title: TextDisplay::new(
                HorizontalAlign::Middle(-INVENTORY_X_OFFSET),
                VerticalAlign::Middle(INVENTORY_UNIT_TITLE_OFFSET),
            ),
            tile_title: TextDisplay::new(
                HorizontalAlign::Middle(INVENTORY_X_OFFSET),
                VerticalAlign::Middle(INVENTORY_TILE_TITLE_OFFSET),
            ),
    ...
    pub fn try_handle_inventory_keypress(
        &mut self,
        key: VirtualKeyCode,
        client: &Client,
        selected: u8,
        ...
        match key {
            ...
            | VirtualKeyCode::A
            | VirtualKeyCode::D => self.toggle_active_inventory(),
    ...
    pub fn draw(&mut self, ctx: &mut Context, selected_id: Option<u8>, map: &Map, verses_ai: bool) {
        ...
        self.game_info
            .set_text(format!("Turn {} - {}\n{}", map.turn(), side, selected));

        // Set the inventory
        if self.inventory_active {
            let info = selected_id
                .and_then(|selected| map.units.get(selected))
                .map(|unit| InventoryInfo::new(unit, map));
            ...
            if let Some(info) = info {
                ...
                self.unit_inventory
                    .set_entries(vec_or_default(info.items, || ListItem::new("No items")));
                self.tile_inventory
                    .set_entries(vec_or_default(info.ground, || ListItem::new("No items")));
            }
        }
